...
public class RemoveWaiverHandler extends CommandHandler<RemoveWaiverCommand, Void> {

    private final ProductConfigurationApi productConfigurationApi;

    public RemoveWaiverHandler(ProductConfigurationApi productConfigurationApi) {
        this.productConfigurationApi = productConfigurationApi;
    }

    @Override
    protected Mono<Void> doHandle(RemoveWaiverCommand cmd) {
        return productConfigurationApi.deleteConfiguration(cmd.productId(), cmd.waiverId(), UUID.randomUUID().toString());
    }
}
